...
static inline void ProfileTickersUpdate()
{
    for (int i = 0; i < globalProfileRecords.num; i++)
    {
        ...
        if (record && record->name)
        {
            ...
            {
                double time = (double)((record->samples[j].end.QuadPart - record->samples[j].start.QuadPart) *
                ...
                globalProfileTickers.times[i] =
                    ...
                    (1.0 - pow(globalProfileTickers.alpha, globalProfileTickers.iterations[i]));
            }
...
#define PROFILE_BEGIN(NAME)                                                                                            \
    static ProfileRecord __PROFILE_RECORD_##NAME;                                                                      \
    ProfileRecordBegin(&__PROFILE_RECORD_##NAME, #NAME);
...
// Quaternion exponent, log, and angle axis functions (see:
...
static inline Quaternion QuaternionInterpolateCubic(Quaternion r0, Quaternion r1, Quaternion r2, Quaternion r3,
                                                    float alpha)
{
    Vector3 r1r0 = QuaternionToScaledAngleAxis(QuaternionAbsolute(QuaternionMultiply(r1, QuaternionInvert(r0))));
...
// Finds an argument on the command line with the given name (in the format "--argName=argValue") and returns the
// argValue as a string
...
static inline bool ArgFlag(int argc, char **argv, const char *name)
{
    for (int i = 1; i < argc; i++)
    {
        if (strlen(argv[i]) > 3 && argv[i][0] == '-' && argv[i][1] == '-' && strstr(argv[i] + 2, name) == argv[i] + 2)
        {
            return true;
        }
    }
...
static inline int ArgEnum(int argc, char **argv, const char *name, int optionCount, const char *options[],
                          int defaultValue)
{
    char *value = ArgFind(argc, argv, name);
    if (!value)
...
static inline void OrbitCameraUpdate(OrbitCamera *camera, Vector3 target, float azimuthDelta, float altitudeDelta,
                                     float offsetDeltaX, float offsetDeltaY, float mouseWheel, float dt)
{
...
static inline bool LoadCharacterGltf(CharacterModel *model, const char *arg_mesh)
{
    ...
    {
        fprintf(stderr,
                "[ERROR - BVHVIEW] The mesh path (--mesh argument if specified) must specify a \".gltf\" file.\n");
        return false;
    }
...
static bool BVHParseChannelEnum(char *channel, Parser *par, const char *channelName, char channelValue)
{
    BVHParseWhitespace(par);
...
static bool BVHParse(BVHData *bvh, Parser *par)
{
    ...
    if (!BVHParseFrames(bvh, par))
    {
        return false;
    }
    if (!BVHParseFrameTime(bvh, par))
    {
        return false;
    }
    if (!BVHParseMotionData(bvh, par))
    {
        return false;
    }
        ...
        for (int c = 0; c < bvh->joints[i].channelCount; c++)
        {
            ...
            case CHANNEL_X_ROTATION:
                rotation = QuaternionMultiply(
                    rotation, QuaternionFromAxisAngle((Vector3){1, 0, 0},
            ...
            case CHANNEL_Y_ROTATION:
                rotation = QuaternionMultiply(
                    rotation, QuaternionFromAxisAngle((Vector3){0, 1, 0},
                                                      DEG2RAD * bvh->motionData[frame * bvh->channelCount + offset]));
                offset++;
                break;
            ...
            case CHANNEL_Z_ROTATION:
                rotation = QuaternionMultiply(
                    rotation, QuaternionFromAxisAngle((Vector3){0, 0, 1},
                                                      DEG2RAD * bvh->motionData[frame * bvh->channelCount + offset]));
                offset++;
                break;
            }
...
static void TransformDataSampleFrameLinear(TransformData *data, TransformData *tmp0, TransformData *tmp1, BVHData *bvh,
...
static void TransformDataSampleFrameCubic(TransformData *data, TransformData *tmp0, TransformData *tmp1,
                                          TransformData *tmp2, TransformData *tmp3, BVHData *bvh, float time,
                                          float scale)
{
    const float alpha = fmod(time / bvh->frameTime, 1.0f);
    ...
    for (int i = 0; i < data->jointCount; i++)
    {
        ...
        else
        {
            data->globalPositions[i] = Vector3Add(
                Vector3RotateByQuaternion(data->localPositions[i], data->globalRotations[p]), data->globalPositions[p]);
...
static bool CharacterDataLoadFromFile(CharacterData *data, Model *model, Sound *audio, const char *path, char *errMsg,
                                      int errMsgSize)
{
...
static inline float NearestPointOnLineSegment(Vector3 lineStart, Vector3 lineVector, Vector3 point)
{
...
static inline void NearestPointBetweenLineSegments(float *nearestTime0, float *nearestTime1, Vector3 line0Start,
                                                   Vector3 line0End, Vector3 line1Start, Vector3 line1End)
{
...
static inline float NearestPointBetweenLineSegmentAndPlane(Vector3 lineStart, Vector3 lineVector, Vector3 planePosition,
                                                           Vector3 planeNormal)
{
...
static inline void NearestPointBetweenLineSegmentAndGroundSegment(float *nearestTimeOnLine,
                                                                  Vector3 *nearestPointOnGround, Vector3 lineStart,
                                                                  Vector3 lineEnd, Vector3 groundMins,
                                                                  Vector3 groundMaxs)
{
...
static inline Vector3 ProjectPointOntoSweptLine(Vector3 sweptLineStart, Vector3 sweptLineVec, Vector3 sweptLineSweepVec,
                                                Vector3 position)
{
...
static inline void NearestPointBetweenLineSegmentAndSweptLine(float *nearestTimeOnLine,
                                                              Vector3 *nearestPointOnSweptLine, Vector3 lineStart,
                                                              Vector3 lineEnd, Vector3 sweptLineStart,
...
static inline float SphereDirectionalOcclusion(Vector3 pos, Vector3 sphere, float radius, Vector3 coneDir,
                                               float coneAngle)
{
...
static inline float CapsuleDirectionalOcclusion(Vector3 pos, Vector3 capStart, Vector3 capVec, float capRadius,
...
static void CapsuleDataInit(CapsuleData *data)
{
    ...
    data->aoLookupImage = (Image){.data = calloc(32 * 32, 1),
                                  .width = 32,
                                  .height = 32,
    ...
    data->shadowLookupImage = (Image){.data = calloc(256 * 128, 1),
                                      .width = 256,
                                      .height = 128,
                                      .format = PIXELFORMAT_UNCOMPRESSED_GRAYSCALE,
                                      .mipmaps = 1};
    ...
    data->shadowLookupResolution =
        (Vector2){(float)data->shadowLookupImage.width, (float)data->shadowLookupImage.height};
...
static void CapsuleDataAppendFromTransformData(CapsuleData *data, TransformData *xforms, float maxCapsuleRadius,
                                               Color color, float opacity, bool ignoreEndSite)
{
...
static void CapsuleDataUpdateAOCapsulesForGroundSegment(CapsuleData *data, Vector3 groundSegmentPosition)
{
    ...
    for (int i = 0; i < data->capsuleCount; i++)
    {
        ...
        if (Vector3Distance(groundSegmentPosition, capsulePosition) - sqrtf(2.0f) >
        ...
        float capsuleOcclusion =
            Vector3Distance(groundPoint, capsulePoint) < capsuleRadius
                ? 0.0f
    ...
    for (int i = 0; i < data->aoCapsuleCount; i++)
    {
        ...
        data->aoCapsuleStarts[i] =
            CapsuleStart(data->capsulePositions[j], data->capsuleRotations[j], data->capsuleHalfLengths[j]);
        data->aoCapsuleVectors[i] =
            CapsuleVector(data->capsulePositions[j], data->capsuleRotations[j], data->capsuleHalfLengths[j]);
...
static void CapsuleDataUpdateAOCapsulesForCapsule(CapsuleData *data, int capsuleIndex)
{
    ...
    for (int i = 0; i < data->capsuleCount; i++)
    {
        ...
        float capsuleOcclusion = Vector3Distance(queryPoint, capsulePoint) <= queryCapsuleRadius + capsuleRadius
                                     ? 0.0f
...
static void CapsuleDataUpdateShadowCapsulesForGroundSegment(CapsuleData *data, Vector3 groundSegmentPosition,
                                                            Vector3 lightDir, float lightConeAngle)
{
    ...
    for (int i = 0; i < data->capsuleCount; i++)
    {
        ...
        if (Vector3Distance(groundSegmentPosition, groundCapsuleMid) - sqrtf(2.0f) >
            capsuleHalfLength + maxRatio * capsuleRadius)
        {
            continue;
        }
    ...
    for (int i = 0; i < data->shadowCapsuleCount; i++)
    {
        ...
        data->shadowCapsuleStarts[i] =
            CapsuleStart(data->capsulePositions[j], data->capsuleRotations[j], data->capsuleHalfLengths[j]);
        data->shadowCapsuleVectors[i] =
            CapsuleVector(data->capsulePositions[j], data->capsuleRotations[j], data->capsuleHalfLengths[j]);
...
static void CapsuleDataUpdateShadowCapsulesForCapsule(CapsuleData *data, int capsuleIndex, Vector3 lightDir,
                                                      float lightConeAngle)
{
    ...
    for (int i = 0; i < data->capsuleCount; i++)
    {
        ...
        if (Vector3Distance(queryCapsulePosition, capsuleMid) - queryCapsuleHalfLength - queryCapsuleRadius >
            capsuleHalfLength + maxRatio * capsuleRadius)
        {
            continue;
        }
        ...
        if (Vector3Distance(queryCapsulePoint, nearestRayPoint) - queryCapsuleRadius >
            capsuleHalfLength + maxRatio * capsuleRadius)
        {
            continue;
        }
        ...
        float capsuleOcclusion =
            Vector3Distance(queryCapsulePoint, nearestRayPoint) <= queryCapsuleRadius + capsuleRadius
                ? 0.0f
...
#define GLSL(X)                                                                                                        \
    "#version 300 es\n" GLSL_DEFINE(AO_RATIO_MAX) GLSL_DEFINE(AO_CAPSULES_MAX) GLSL_DEFINE(SHADOW_CAPSULES_MAX)        \
        GLSL_DEFINE(PI) #X
...
static const char *shaderVS = GLSL(
    ...
    uniform int isCapsule; uniform vec3 capsulePosition; uniform vec4 capsuleRotation; uniform float capsuleHalfLength;
    uniform float capsuleRadius;

    uniform mat4 matModel; uniform mat4 matNormal; uniform mat4 matView; uniform mat4 matProjection;

    uniform int isCharacter;

    out vec3 fragPosition; out vec2 fragTexCoord; out vec3 fragNormal;
    ...
    void main() {
        ...
        if (isCapsule == 1)
        {
            fragPosition = Rotate(capsuleRotation, CapsuleStretch(vertexPosition, capsuleHalfLength, capsuleRadius)) +
                           capsulePosition;
...
static const char *shaderFS = GLSL(
    ...
    in vec3 fragPosition; in vec2 fragTexCoord; in vec3 fragNormal;

    uniform vec3 objectColor; uniform float objectSpecularity; uniform float objectGlossiness;
    uniform float objectOpacity;

    uniform int isCapsule; uniform vec3 capsulePosition; uniform vec4 capsuleRotation; uniform float capsuleHalfLength;
    uniform float capsuleRadius; uniform vec3 capsuleStart; uniform vec3 capsuleVector;

    uniform int shadowCapsuleCount; uniform vec3 shadowCapsuleStarts[SHADOW_CAPSULES_MAX];
    uniform vec3 shadowCapsuleVectors[SHADOW_CAPSULES_MAX]; uniform float shadowCapsuleRadii[SHADOW_CAPSULES_MAX];
    uniform sampler2D shadowLookupTable; uniform vec2 shadowLookupResolution;

    uniform int aoCapsuleCount; uniform vec3 aoCapsuleStarts[AO_CAPSULES_MAX];
    uniform vec3 aoCapsuleVectors[AO_CAPSULES_MAX]; uniform float aoCapsuleRadii[AO_CAPSULES_MAX];
    uniform sampler2D aoLookupTable; uniform vec2 aoLookupResolution;

    uniform vec3 cameraPosition;

    uniform float sunStrength; uniform vec3 sunDir; uniform vec3 sunColor; uniform float skyStrength;
    uniform vec3 skyColor; uniform float ambientStrength; uniform float groundStrength; uniform float exposure;

    uniform int isCharacter; uniform sampler2D characterTexture;

    out vec4 finalColor;

    vec3 ToGamma(in vec3 col) { return vec3(pow(col.x, 2.2), pow(col.y, 2.2), pow(col.z, 2.2)); }

    vec3 FromGamma(in vec3 col) { return vec3(pow(col.x, 1.0 / 2.2), pow(col.y, 1.0 / 2.2), pow(col.z, 1.0 / 2.2)); }
    ...
    vec3 Unrotate(in vec4 q, vec3 v) { return Rotate(vec4(-q.x, -q.y, -q.z, q.w), v); }
    ...
    float SphereDirectionalOcclusion(in vec3 pos, in vec3 sphere, in float radius, in vec3 coneDir) {
    ...
    float CapsuleOcclusion(in vec3 pos, in vec3 nor, in vec3 capStart, in vec3 capVec, in float radius) {
        vec3 ba = capVec;
    ...
    float CapsuleDirectionalOcclusion(in vec3 pos, in vec3 capStart, in vec3 capVec, in float capRadius,
                                      in vec3 coneDir) {
        vec3 ba = capVec;
    ...
    vec2 CapsuleUVs(in vec3 pos, in vec3 capPos, in vec4 capRot, in float capHalfLength, in float capRadius,
                    in vec2 scale) {
        vec3 loc = Unrotate(capRot, pos - capPos);

        vec2 limit = vec2(2.0 * capHalfLength + 2.0 * capRadius, PI * capRadius);
    ...
    vec3 CapsuleNormal(in vec3 pos, in vec3 capStart, in vec3 capVec) {
        vec3 ba = capVec;
        vec3 pa = pos - capStart;
    ...
    void main() {
        ...
        if (isCapsule == 1)
        {
            uvs = CapsuleUVs(pos, capsulePosition, capsuleRotation, capsuleHalfLength, capsuleRadius, vec2(4.0, 4.0));
        ...
        {
            sunShadow = min(sunShadow, CapsuleDirectionalOcclusion(pos, shadowCapsuleStarts[i], shadowCapsuleVectors[i],
                                                                   shadowCapsuleRadii[i], sunDir));
        }
        ...
        {
            ambShadow =
                min(ambShadow, CapsuleOcclusion(pos, nor, aoCapsuleStarts[i], aoCapsuleVectors[i], aoCapsuleRadii[i]));
        }
...
static inline void DrawWireFrames(CapsuleData *capsuleData, Color color)
{
    ...
    {
        Vector3 capsuleStart = CapsuleStart(capsuleData->capsulePositions[i], capsuleData->capsuleRotations[i],
                                            capsuleData->capsuleHalfLengths[i]);
        Vector3 capsuleEnd = CapsuleEnd(capsuleData->capsulePositions[i], capsuleData->capsuleRotations[i],
                                        capsuleData->capsuleHalfLengths[i]);
...
static inline void GuiOrbitCamera(OrbitCamera *camera, CharacterData *characterData, int argc, char **argv)
{
    ...
    GuiLabel((Rectangle){30, 80, 150, 20}, TextFormat("Target: [% 5.3f % 5.3f % 5.3f]", camera->cam3d.target.x,
                                                      camera->cam3d.target.y, camera->cam3d.target.z));
    GuiLabel((Rectangle){30, 100, 150, 20},
    ...
    if (characterData->count > 0)
    {
        ...
        GuiComboBox((Rectangle){30, 240, 150, 20}, characterData->jointNamesCombo[characterData->active],
                    &camera->trackBone);
    }
}

static inline void GuiRenderSettings(RenderSettings *settings, CapsuleData *capsuleData, int screenWidth,
                                     int screenHeight)
{
    GuiGroupBox((Rectangle){screenWidth - 260, 10, 240, 430}, "Rendering");

    GuiSliderBar((Rectangle){screenWidth - 160, 20, 100, 20}, "Exposure", TextFormat("%5.2f", settings->exposure),
                 &settings->exposure, 0.0f, 3.0f);

    GuiSliderBar((Rectangle){screenWidth - 160, 50, 100, 20}, "Sun Light",
                 TextFormat("%5.2f", settings->sunLightStrength), &settings->sunLightStrength, 0.0f, 1.0f);

    if (GuiSliderBar((Rectangle){screenWidth - 160, 80, 100, 20}, "Sun Softness",
    ...
    GuiSliderBar((Rectangle){screenWidth - 160, 110, 100, 20}, "Sky Light",
                 TextFormat("%5.2f", settings->skyLightStrength), &settings->skyLightStrength, 0.0f, 1.0f);

    GuiSliderBar((Rectangle){screenWidth - 160, 140, 100, 20}, "Ambient Light",
                 TextFormat("%5.2f", settings->ambientLightStrength), &settings->ambientLightStrength, 0.0f, 2.0f);

    GuiSliderBar((Rectangle){screenWidth - 160, 170, 100, 20}, "Ground Light",
                 TextFormat("%5.2f", settings->groundLightStrength), &settings->groundLightStrength, 0.0f, 0.5f);

    GuiSliderBar((Rectangle){screenWidth - 160, 200, 100, 20}, "Sun Azimuth", TextFormat("%5.2f", settings->sunAzimuth),
                 &settings->sunAzimuth, -PI, PI);

    GuiSliderBar((Rectangle){screenWidth - 160, 230, 100, 20}, "Sun Altitude",
...
static inline void GuiCharacterData(CharacterData *characterData, GuiWindowFileDialogState *fileDialogState,
                                    ScrubberSettings *scrubberSettings, char *errMsg, int argc, char **argv)
{
    ...
    for (int i = 0; i < characterData->count; i++)
    {
        ...
        if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT))
        {
            ...
            if (mousePosition.x > 180 && mousePosition.x < 200 && mousePosition.y > offsetHeight + 40 + i * 30 &&
                mousePosition.y < offsetHeight + 40 + i * 30 + 20)
            {
    ...
    if (characterData->count > 0)
    {
        ...
        GuiSliderBar((Rectangle){70, offsetHeight + 90 + (CHARACTERS_MAX - 1) * 30, 100, 20}, "Radius",
                     TextFormat("%5.2f", characterData->radii[characterData->active]),
                     &characterData->radii[characterData->active], 0.01f, 0.1f);

        GuiSliderBar((Rectangle){70, offsetHeight + 120 + (CHARACTERS_MAX - 1) * 30, 100, 20}, "Opacity",
                     TextFormat("%5.2f", characterData->opacities[characterData->active]),
                     &characterData->opacities[characterData->active], 0.0f, 1.0f);
    }
}

static inline void GuiScrubberSettings(ScrubberSettings *settings, CharacterData *characterData, int screenWidth,
                                       int screenHeight)
{
    if (characterData->count == 0)
    {
    ...
    GuiComboBox((Rectangle){screenWidth / 2 - 240, screenHeight - 80, 100, 20}, "Nearest;Linear;Cubic",
                &settings->sampleMode);
    ...
    GuiToggle((Rectangle){screenWidth / 2 + 50, screenHeight - 80, 30, 20}, "0.1x", &speed01x);
    if (speed01x)
    {
        settings->playSpeed = 0.1f;
    }
    bool speed05x = settings->playSpeed == 0.5f;
    GuiToggle((Rectangle){screenWidth / 2 + 90, screenHeight - 80, 30, 20}, "0.5x", &speed05x);
    if (speed05x)
    {
        settings->playSpeed = 0.5f;
    }
    bool speed1x = settings->playSpeed == 1.0f;
    GuiToggle((Rectangle){screenWidth / 2 + 130, screenHeight - 80, 30, 20}, "1x", &speed1x);
    if (speed1x)
    {
        settings->playSpeed = 1.0f;
    }
    bool speed2x = settings->playSpeed == 2.0f;
    GuiToggle((Rectangle){screenWidth / 2 + 170, screenHeight - 80, 30, 20}, "2x", &speed2x);
    if (speed2x)
    {
        settings->playSpeed = 2.0f;
    }
    bool speed4x = settings->playSpeed == 4.0f;
    GuiToggle((Rectangle){screenWidth / 2 + 210, screenHeight - 80, 30, 20}, "4x", &speed4x);
    if (speed4x)
    {
        settings->playSpeed = 4.0f;
    }
    GuiSliderBar((Rectangle){screenWidth / 2 + 250, screenHeight - 80, 70, 20}, "",
    ...
    if (GuiValueBox((Rectangle){screenWidth / 2 - 540, screenHeight - 80, 50, 20}, "Min   ", &settings->frameMinSelect,
    ...
    if (GuiValueBox((Rectangle){screenWidth / 2 + 470, screenHeight - 80, 50, 20}, "Max   ", &settings->frameMaxSelect,
    ...
    GuiSliderBar((Rectangle){screenWidth / 2 - 540, screenHeight - 50, 1080, 20},
    ...
    settings->events.scrubberBeingDragged =
        (frame !=
         prevFrame); // Couldn't find a more robust way; this will trigger even if changes are not made via the GUI
...
static void ApplicationUpdate(void *voidApplicationState)
{
    ...
    Frustum frustum =
        FrustumFromCameraMatrices(GetCameraProjectionMatrix(&app->camera.cam3d, app->screenHeight / app->screenWidth),
    ...
    Vector3 sunLightPosition =
        Vector3RotateByQuaternion((Vector3){0.0f, 0.0f, 1.0f},
                                  QuaternionFromAxisAngle((Vector3){0.0f, 1.0f, 0.0f}, app->renderSettings.sunAzimuth));
    ...
    if (app->renderSettings.drawChecker)
    {
        ...
        for (int i = 0; i < app->ground.gridX; i++)
        {
            for (int j = 0; j < app->ground.gridZ; j++)
            {
                ...
                {
                    groundSegmentPosition.x =
                        (((float)i / (app->ground.gridX - 1)) - 0.5f) * (app->ground.gridX - 1) * app->ground.cellWidth;
                }
                if (app->ground.gridZ >= 2)
                {
                    groundSegmentPosition.z =
                        (((float)j / (app->ground.gridZ - 1)) - 0.5f) * (app->ground.gridZ - 1) * app->ground.cellWidth;
                }

                // @note Originally sqrtf(app->ground.cellWidth) but caused culling on large cell width. Should it be
                // sqrtf?
                ...
                SetShaderValueV(app->shader, app->uniforms.aoCapsuleStarts, app->capsuleData.aoCapsuleStarts,
                                SHADER_UNIFORM_VEC3, aoCapsuleCount);
                SetShaderValueV(app->shader, app->uniforms.aoCapsuleVectors, app->capsuleData.aoCapsuleVectors,
                                SHADER_UNIFORM_VEC3, aoCapsuleCount);
                SetShaderValueV(app->shader, app->uniforms.aoCapsuleRadii, app->capsuleData.aoCapsuleRadii,
                ...
                SetShaderValueV(app->shader, app->uniforms.shadowCapsuleStarts, app->capsuleData.shadowCapsuleStarts,
                                SHADER_UNIFORM_VEC3, shadowCapsuleCount);
                SetShaderValueV(app->shader, app->uniforms.shadowCapsuleVectors, app->capsuleData.shadowCapsuleVectors,
                                SHADER_UNIFORM_VEC3, shadowCapsuleCount);
                SetShaderValueV(app->shader, app->uniforms.shadowCapsuleRadii, app->capsuleData.shadowCapsuleRadii,
    ...
    if (app->renderSettings.drawCapsules)
    {
        ...
        for (int i = 0; i < app->capsuleData.capsuleCount; i++)
        {
            ...
            app->capsuleData.capsuleSort[i].value =
                Vector3Distance(app->camera.cam3d.position, app->capsuleData.capsulePositions[i]);
        }
    ...
    {
        DrawTransform((Vector3){0.0f, 0.01f, 0.0f}, QuaternionIdentity(), 1.0f);
    }
    ...
    if (app->renderSettings.drawUI && !app->recording.enabled)
    {
        ...
        if (app->characterData.count == 0)
        {
            DrawText("Drag and Drop .bvh files to open them.", app->screenWidth / 2 - 300, app->screenHeight / 2 - 15,
                     30, DARKGRAY);
        }
        ...
        if (app->characterData.colorPickerActive)
        {
            ...
            GuiColorPicker((Rectangle){app->screenWidth - 165, 465, 110, 110}, NULL,
    ...
    for (int i = 0; i < globalProfileRecords.num; i++)
    {
        ...
        GuiLabel((Rectangle){550, 10 + (float)i * 20, 100, 20},
...
int main(int argc, char **argv)
{
    ...
    if (argvBvhPath)
    {
        if (realpath(argvBvhPath, bvhPathAbsolute) != NULL)
        {
            ...
            CharacterDataLoadFromFile(&app.characterData, &app.characterModel.model, audio, bvhPathAbsolute, app.errMsg,
                                      512);
        }
    }
