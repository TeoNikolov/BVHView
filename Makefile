...
ifeq ($(PLATFORM), PLATFORM_DESKTOP)
    ...
    ifeq ($(findstring MINGW,$(PLATFORM_OS)),MINGW)
        ...
        LIBRARY_DIR = -L $(RAYLIB_DIR)/raylib/src
        ifeq ($(BUILD_MODE),RELEASE)
            CFLAGS ?= ./res/bvhview.res $(DEFINES) -Wall $(MWINDOWS_FLAG) -D NDEBUG -O3 $(INCLUDE_DIR) $(LIBRARY_DIR)
        else
            CFLAGS ?= ./res/bvhview.res $(DEFINES) -Wall -pg $(INCLUDE_DIR) $(LIBRARY_DIR) -ggdb
        endif
...
bvhview: src/bvhview.c src/additions.c src/external/cwalk.c
	...
	$(CC) -o build/BVHView/$@$(EXT) src/bvhview.c src/additions.c src/external/cwalk.c $(CFLAGS) $(LIBS)
